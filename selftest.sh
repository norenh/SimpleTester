...
for FILE in test/config_broken/config*; do 
...
for FILE in test/script_broken/script*; do
...
# Test inputtest with edge, check for 0 exit value, SUCCESS line in end
./run.sh -b edge -h -e ERROR test/inputtest/inputconfig.txt "file://$(pwd)/test/inputtest/inputtest.html" test/inputtest/script1.txt &> "${OUTPUTFILE}";
test_check "Inputtest - regular (edge)" 0 "^SUCCESS:" "$?" "true"
...
rm -f "ERROR.png"
for FILE in test/demopage/fail/*.txt; do
...
for FILE in test/inputtest/fail/*.txt; do
